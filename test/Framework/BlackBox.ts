...
import { Selection, TextDocument, TextEditor, extensions } from "vscode";
  ...
  while (true) {
    ...
    cleanText = cleanText.replace(
      ...
      (match: string, content: string, startOffset: number) => {
        ...
        const startCharacter = getCharacter(cleanText, startOffset);
...
export const run = (testCase: TestCase, before?: (textEditor: TextEditor) => void) => {
  ...
  const { getCurrentMode } = extensions.getExtension("grzegorzkiselev.vims")?.exports;
